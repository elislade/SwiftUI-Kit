...
@dynamicMemberLookup public struct PresentationValue<Metadata: Equatable>: Equatable, PresentationValueConformable {
    ...
    public static func == (lhs: PresentationValue, rhs: PresentationValue) -> Bool {
        ...
        let tag = lhs.tag == rhs.tag
        let anchor = lhs.anchor == rhs.anchor
        let meta = lhs.metadata == rhs.metadata
        return id && tag && anchor && meta
    }
...
/// - Note: This behaviour is only for id of the presentation preference if any other component of the preference value changes such as `tag`, `boundsAnchor`, or `metadata`, the view will be re-evaluated.
